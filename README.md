...
**Constantes volontairement basses en l'état** (`TOP_N`, `CANDIDATES_SIZE` dans `tripadvisor_import.py`) : réglées à des valeurs faibles pour limiter le nombre de requêtes envoyées à l'API pendant les tests/le développement (chaque import synchrone peut vite représenter plusieurs dizaines d'appels). À ajuster à la hausse selon les besoins réels (démo, production) une fois le comportement validé.
